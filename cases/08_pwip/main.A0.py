...
def main():
    ...
    def pde_loss(kp, u, A, A0, Kr):
        ...
        res0 = dAdt + A0m[None,:] * dudx
        res1 = dudt + (dAdx - dA0dx[None,:] - (Amm - A0m[None,:]) * dkpdx[None,:] / kpm[None,:]) / (rho * kpm[None,:]) + Kr * umm
    ...
    if fit_Kr:
        Kr = torch.tensor([float(Kr)], requires_grad=True)
    ...
    for epoch in epochs:
        optim.zero_grad()
        ploss = pde_loss(kp=kp, u=u, A=A, A0=A0_, Kr=Kr)
        dloss = data_loss(u, A)
        loss = ploss + dloss
        loss.backward()
        optim.step()
